		...
		if ( 'shipping_address' === $column ) {
			...
			if ( ! empty( $codes ) ) {
				...
				foreach ( $codes as $code ) {
					$tracking_codes[] = '<a href="#" aria-label="' . esc_attr__( 'Tracking code', 'correios-for-woocommerce' ) . '">' . esc_html( $code ) . '</a>';
				}
	...
	public function metabox_content( $post ) {
		...
		wp_localize_script(
			...
			array(
				'order_id' => $post->ID,
				'i18n'     => array(
					'removeQuestion' => esc_js( __( 'Are you sure you want to remove this tracking code?', 'correios-for-woocommerce' ) ),
				),
				'nonces'   => array(
